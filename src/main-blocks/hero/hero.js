...
console.log("getTrailer from trailer js", getTrailer);
...
function renderHero({ results }) {
  ...
  const watchTrailerButton = document.querySelector(".hero-button-js");
  let trailerOpened = false;

  watchTrailerButton.addEventListener("click", () => {
    if (!trailerOpened) {
      trailerOpened = true;
      getTrailer(movieTheDay.id)
        .then(trailer => {
